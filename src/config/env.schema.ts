...
export const envSchema = z
  .object({
    ...
    FILE_STORAGE_PATH: z.string().min(1).optional(),
  })
  .superRefine((env, ctx) => {
    ...
    if (
      ...
      (!env.FILE_STORAGE_PATH || !isAbsolute(env.FILE_STORAGE_PATH))
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['FILE_STORAGE_PATH'],
        message:
          'FILE_STORAGE_PATH must be an absolute persistent path in production',
      });
    }
  });
